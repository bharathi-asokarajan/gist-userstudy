...
folder_path = os.path.join(os.path.expanduser("~"), "Desktop", "gist_outliers_zscore_jitter1")
...
for num_points in num_points_list:
    for num_out in num_outliers:
        if num_out == 0:
            ...
            # Save plot as PNG
            filename = f"plot_{num_points}_{num_out}.png"
            ...
            for sd in sd_away:
                ...
                # Save plot as PNG
                filename = f"plot_{num_points}_outliers_{num_out}_sd_{sd}.png"
                filepath = os.path.join(folder_path, filename)
                plt.savefig(filepath)
