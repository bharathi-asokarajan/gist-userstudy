...
folder_path = os.path.join(os.path.expanduser("~"), "Desktop", "gist_clusters_nocluster_jitter2")
...
cluster_covariances = [covariance_cluster1]

num_points_per_cluster_list = [100, 500, 1000]
...
for num_points_per_cluster in num_points_per_cluster_list:
    ...
    # Add jitter
    data[:, 0] += np.random.uniform(-jitter_range, jitter_range, size=num_points_per_cluster)
    data[:, 1] += np.random.uniform(-jitter_range, jitter_range, size=num_points_per_cluster)
    ...
    title = f'plot_{num_points_per_cluster}_{num_clusters}_clusters_0_jitter2.png'
